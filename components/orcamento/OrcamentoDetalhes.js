...
import { faSpinner, faPlus, faArrowLeft, faPen, faTrash, faGripVertical } from '@fortawesome/free-solid-svg-icons';
        ...
        <>
            ...
            <div className="space-y-6">
                <div className="sticky top-16 z-20 bg-white py-4 flex justify-between items-center mb-4 border-b border-gray-200">
                    ...
                    <button onClick={() => handleOpenModal()} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold px-4 py-2 rounded-md shadow-sm flex items-center gap-2 transition-colors">
                                                            ...
                                                            <Fragment key={subgroup.key}>
                                                                ...
                                                                {subgroup.items.map((item, itemIndex) => (
                                                                    <tr key={item.id} draggable onDragStart={(e) => handleDragStart(e, item)} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, item)} className="hover:bg-gray-50 cursor-grab">
                                                                        ...
                                                                        <td className="px-6 py-4 text-sm">{item.descricao}</td><td className="px-6 py-4 text-sm">{item.unidade}</td><td className="px-6 py-4 text-sm text-center">{item.quantidade}</td><td className="px-6 py-4 text-sm text-right">{formatCurrency(item.preco_unitario)}</td><td className="px-6 py-4 text-sm text-right font-semibold">{formatCurrency(item.custo_total)}</td><td className="px-6 py-4 text-sm text-right font-semibold text-blue-600">{formatPercentage(item.custo_total)}</td><td className="px-6 py-4 text-sm text-center"><div className="flex items-center justify-center gap-3"><button onClick={() => handleOpenModal(item)} title="Editar" className="text-blue-500 hover:text-blue-700 transition-colors"><FontAwesomeIcon icon={faPen} /></button><button onClick={() => handleDeleteItem(item.id)} title="Excluir" className="text-gray-500 hover:text-red-700 transition-colors"><FontAwesomeIcon icon={faTrash} /></button></div></td>
                                                                    </tr>
                                                                ))}
                                                            </Fragment>
                                                        );
