                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {empresas.map((empresa) => (
                        ...
                        >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{empresa.nome_fantasia}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{empresa.cnpj}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Link href={`/empresas/editar/${empresa.id}`} onClick={handleEditClick} title="Editar" className="text-blue-500 hover:text-blue-700 transition-colors">
                                        <FontAwesomeIcon icon={faPen} />
                                    </Link>
                                    {isAdmin && (
                                        <button onClick={(e) => handleDelete(e, empresa.id, empresa.razao_social)} title="Excluir" className="text-gray-500 hover:text-red-700 transition-colors">
