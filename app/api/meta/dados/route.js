...
export async function GET(request) {
    ...
    try {
        ...
        // 5. BUSCAR DADOS (Se tiver Ad Account ID salvo, usa. Se não, tenta descobrir)
        // Nota: O ideal é salvarmos o ad_account_id na hora da conexão. 
        // Se não tivermos, vamos pegar a primeira conta de anúncios associada ao usuário (fallback).
        
        let adAccountId = integracao.ad_account_id;

        if (!adAccountId) {
            // Tenta descobrir a conta de anúncios (Fallback)
            const me = new (require('facebook-nodejs-business-sdk').User)('me');
            const adAccounts = await me.getAdAccounts(['id', 'name', 'currency']);
            if (adAccounts.length > 0) {
                adAccountId = adAccounts[0].id;
            } else {
                return NextResponse.json({ data: [] }); // Sem conta de anúncios
            }
        }
