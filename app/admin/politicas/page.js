...
import { faPlus, faCheckCircle, faFileAlt, faSpinner, faWarning } from '@fortawesome/free-solid-svg-icons';
...
export default function GestaoPoliticasPage() {
    ...
    const createPolicyMutation = useMutation({
        mutationFn: async (payload) => {
            ...
            // Insere a nova política com is_active true
            const { data, error } = await supabase
                .from('politicas_plataforma')
        ...
        onSuccess: () => {
            toast.success("Nova política publicada e ativada com sucesso!");
            queryClient.invalidateQueries(['politicas_admin']);
            setFormData({ tipo: 'termos_uso', versao: '', titulo: '', conteudo: '' });
            setIsCreating(false);
        },
        onError: (err) => {
            toast.error(`Falha ao publicar política: ${err.message}`);
        }
    });
                ...
                <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 shadow-xl animate-fade-in relative overflow-hidden">
                    ...
                    <form onSubmit={handleCreate} className="space-y-4">
                        ...
                        <div className="flex items-center justify-end gap-3 pt-2">
                            ...
                            <button type="submit" disabled={createPolicyMutation.isPending} className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2 rounded-lg font-medium text-sm transition-colors flex items-center gap-2">
                                {createPolicyMutation.isPending ? <FontAwesomeIcon icon={faSpinner} spin /> : "Publicar Edital / Política"}
                            </button>
                        </div>
                    </form>
                </div>
                        ...
                        {politicas.map((pol) => (
                            <div key={pol.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-800/30 transition-colors">
                                ...
                                <div className="flex items-center gap-2">
                                    <button className="text-slate-400 hover:text-slate-200 text-sm font-medium px-3 py-1.5 rounded-md hover:bg-slate-800 transition-colors">
                                        Ler Texto Completo
                                    </button>
                                </div>
                            </div>
